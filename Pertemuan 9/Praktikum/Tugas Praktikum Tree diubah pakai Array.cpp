...
int main (){
	...
	int arrayTree[] = {1,2,3,4,5};
	//Membuat simpul-simpul pohon
	TreeNode* root = new TreeNode(arrayTree[0]);
	root->left = new TreeNode(arrayTree[1]);
	root->right = new TreeNode(arrayTree[2]);
	root->left->left = new TreeNode(arrayTree[3]);
	root->left->right = new TreeNode(arrayTree[4]);
